...
(() => {
  ...
  #dog-actions { position: fixed; left: 50%; right:auto; top:auto; bottom:max(78px, calc(env(safe-area-inset-bottom) + 72px)); display: none; align-items:center; justify-content:center; gap: 8px; padding: 8px; max-width: calc(100vw - 20px); border-radius: 999px; background: rgba(10,15,28,.82); backdrop-filter: blur(24px) saturate(1.75); -webkit-backdrop-filter: blur(24px) saturate(1.75); box-shadow: 0 16px 42px rgba(0,0,0,.42); border: 1px solid rgba(255,255,255,.20); transform: translateX(-50%); animation: dogToolbar .18s ease-out; }
  #dog-actions button { border: 0; border-radius: 999px; color: #fff; background: rgba(255,255,255,.13); padding: 11px 15px; font-size: 14px; font-weight: 900; white-space: nowrap; display:flex; align-items:center; gap:5px; -webkit-user-select:none; user-select:none; }
  #dog-actions button.primary { color:#17110a; background: linear-gradient(135deg,#fff3a5,#ffc842); box-shadow: inset 0 1px 0 rgba(255,255,255,.62), 0 6px 18px rgba(255,196,49,.26); }
  #dog-actions .count { color:rgba(255,255,255,.62); font-size:11px; font-weight:800; padding:0 4px; }
  #dog-translation { position: fixed; display:none; left:12px; right:12px; bottom:max(12px, env(safe-area-inset-bottom)); width:auto; max-height:min(58vh, 520px); overflow:auto; color:#f8fbff; background: rgba(11,16,29,.91); border:1px solid rgba(255,255,255,.17); box-shadow:0 -16px 52px rgba(0,0,0,.45); backdrop-filter: blur(24px) saturate(1.55); -webkit-backdrop-filter: blur(24px) saturate(1.55); border-radius:24px; padding:14px 15px 15px; line-height:1.54; transform:none; animation: dogSlide .18s ease-out; }
  ...
  #dog-translation .origin { max-height:96px; overflow:auto; color:rgba(255,255,255,.55); font-size:12px; white-space:pre-wrap; word-break:break-word; padding:9px 10px; border-radius:14px; background:rgba(255,255,255,.055); }
  ...
  #dog-card-panel { position:fixed; left:12px; right:12px; bottom:max(12px, env(safe-area-inset-bottom)); display:none; color:#fff; background:rgba(8,12,23,.93); border:1px solid rgba(255,255,255,.16); box-shadow:0 -14px 54px rgba(0,0,0,.48); backdrop-filter:blur(26px) saturate(1.45); -webkit-backdrop-filter:blur(26px) saturate(1.45); border-radius:28px; overflow:hidden; animation: dogSlide .2s ease-out; }
  ...
  #dog-card-panel .themes { display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:10px; padding:12px 13px 15px; max-height:min(54vh, 480px); overflow:auto; }
  ...
  #dog-error-panel { position: fixed; left: 12px; right: 12px; bottom: max(12px, env(safe-area-inset-bottom)); display:none; color:#f7f9ff; background: rgba(15,6,10,.94); border:1px solid rgba(255,110,110,.35); box-shadow:0 -12px 54px rgba(0,0,0,.48); backdrop-filter: blur(24px) saturate(1.4); -webkit-backdrop-filter: blur(24px) saturate(1.4); border-radius: 24px; overflow:hidden; animation: dogSlide .18s ease-out; }
  #dog-error-panel .head { display:flex; align-items:center; justify-content:space-between; padding:14px 15px 10px; border-bottom:1px solid rgba(255,255,255,.08); background:linear-gradient(135deg,rgba(255,63,93,.26),rgba(255,190,80,.12)); }
  ...
  #dog-error-panel .list { max-height: min(56vh, 520px); overflow:auto; padding:10px 12px 14px; }
  #dog-error-panel .item { border:1px solid rgba(255,255,255,.12); background:rgba(255,255,255,.06); border-radius:18px; padding:12px; margin:8px 0; }
  #dog-error-panel .item .t { color:#ffdf73; font-weight:900; font-size:14px; margin-bottom:5px; }
  #dog-error-panel .item .d { font-size:13px; color:rgba(255,255,255,.86); line-height:1.5; white-space:pre-wrap; word-break:break-word; }
  ...
  #dog-card-toast { position:fixed; left:50%; bottom:max(138px, calc(env(safe-area-inset-bottom) + 118px)); transform:translateX(-50%); display:none; color:#fff; background:rgba(10,15,25,.88); border:1px solid rgba(255,255,255,.16); border-radius:999px; padding:10px 14px; box-shadow:0 10px 28px rgba(0,0,0,.35); backdrop-filter:blur(18px); -webkit-backdrop-filter:blur(18px); font-size:13px; font-weight:850; }
  @keyframes dogToolbar { from { opacity:0; transform:translateX(-50%) translateY(12px) scale(.96); } to { opacity:1; transform:translateX(-50%) translateY(0) scale(1); } }
  ...
  actions.innerHTML = `<button data-act="translate">🌐 翻译</button><button data-act="card" class="primary">🔖 生成卡片</button><span class="count"></span>`;
  ...
  cardPanel.innerHTML = `<div class="head"><div><b>🔖 选择卡片主题</b><span>选好后会自动打开系统分享</span></div><button data-close>收起</button></div><div class="themes"></div>`;
  document.documentElement.appendChild(cardPanel);

  const errPanel = document.createElement('div');
  errPanel.id = 'dog-error-panel';
  errPanel.innerHTML = `<div class="head"><b>🩺 自动错误码翻译</b><button data-close>收起</button></div><div class="list"></div>`;
  ...
  function withNative(action, payload, cb) {
    ...
    setTimeout(() => {
      if (callbacks.has(requestId)) {
        ...
        cb && cb({ ok:false, error:'原生端没有返回结果，请重试。' });
      }
    }, action === 'makeCard' ? 12000 : 18000);
  }
  ...
  function setTranslationPanel(origin, body, loading) {
  ...
  function translate(text) {
    ...
    if (isMostlyChinese(value)) {
      setTranslationPanel(value, value, false);
    ...
    if (cached) {
      setTranslationPanel(value, cached, false);
      showToast('已使用翻译缓存');
      return;
    }
    setTranslationPanel(value, '正在调用 Microsoft Edge 翻译引擎…', true);
    withNative('translate', { text: value }, (res) => {
      if (res && res.ok) {
        const translated = res.text || '';
        localTranslateCache.set(key, translated);
        setTranslationPanel(value, translated, false);
      } else {
        setTranslationPanel(value, (res && res.error) || '翻译失败，请稍后再试。', false);
      }
    });
  }
  ...
  function plainRect(rect) {
    return rect ? { left: rect.left, top: rect.top, width: rect.width, height: rect.height } : { left: 0, top: 0, width: 1, height: 1 };
  }
  ...
  function currentSelectionInfo() {
    ...
    return { text, rect: plainRect(rect), isAI: ai, character: characterName(mes), length: text.length };
  }
  ...
  document.addEventListener('selectionchange', () => scheduleSelectionUpdate(190), true);
  document.addEventListener('touchend', () => scheduleSelectionUpdate(280), true);
  document.addEventListener('mouseup', () => scheduleSelectionUpdate(140), true);
  document.addEventListener('contextmenu', () => scheduleSelectionUpdate(120), true);
  ...
  actions.addEventListener('pointerdown', ev => { ev.stopPropagation(); }, true);
  cardPanel.addEventListener('pointerdown', ev => { ev.stopPropagation(); }, true);
  trans.addEventListener('pointerdown', ev => { ev.stopPropagation(); }, true);
  ...
  actions.addEventListener('click', (ev) => {
    ...
    const info = activeSelection || currentSelectionInfo();
    if (!btn || !info) return;
    ev.preventDefault();
    ev.stopPropagation();
    if (btn.dataset.act === 'translate') {
      ...
      pendingCard = {
        text: info.text,
        character: info.character || 'AI Message'
      };
  ...
  cardPanel.addEventListener('click', (ev) => {
    ...
    if (!btn) return;
    const data = pendingCard || activeSelection;
    if (!data || !(data.text || '').trim()) {
      showToast('没有可用选区，请重新选中 AI 回复文字。');
    ...
    const payload = { text: data.text, character: data.character || 'AI Message', theme };
    try { window.getSelection()?.removeAllRanges?.(); } catch (_) {}
    cardPanel.style.display = 'none';
    showToast('正在生成卡片…');
    setTimeout(() => {
      withNative('makeCard', payload, (res) => {
        if (res && res.ok) showToast('卡片已生成，正在打开分享。');
        else showToast((res && res.error) || '卡片生成失败，请重试。');
      });
    }, 90);
  });
  ...
  function visible(el) {
    if (!el || el.closest?.('#dog-error-panel,#dog-actions,#dog-translation,#dog-card-panel,#dog-card-toast')) return false;
  ...
  function likelyError(text) {
    return /(error|failed|fail|exception|timeout|timed out|unauthorized|forbidden|quota|rate limit|invalid|denied|not found|bad gateway|service unavailable|network|cors|ssl|tls|401|403|404|408|413|422|429|500|502|503|504|520|521|522|524|错误|失败|异常|超时|无效|禁止|拒绝|额度|频率|连接|中断)/i.test(text || '');
  }

  function popupLike(el) {
    const cls = ((el.className || '') + ' ' + (el.id || '')).toString().toLowerCase();
    if (/toast|error|danger|warning|swal|popup|alert|notify|notification|red/.test(cls)) return true;
    const st = getComputedStyle(el);
    if (st.position === 'fixed' || st.position === 'sticky') return true;
    const bg = st.backgroundColor || '';
    return /rgb\((1[5-9]\d|2[0-5]\d),\s*[0-9]{1,3},\s*[0-9]{1,3}/.test(bg);
  }

  function collectErrors(auto = false) {
    const selectors = auto
      ? '.toast,.toast-message,.toast-error,.toast-warning,#toast-container .toast,.swal2-popup,.popup,.error,.warning,.errorMessage,[role="alert"],[class*="error"],[class*="Error"],[class*="toast"],[class*="Toast"]'
      : '.toast,.toast-message,.toast-error,.toast-warning,#toast-container .toast,.error,.warning,.errorMessage,.swal2-popup,.popup,pre,code,[role="alert"],[class*="error"],[class*="Error"],[class*="toast"],[class*="Toast"]';
    const list = [];
    document.querySelectorAll(selectors).forEach(el => {
      if (!visible(el)) return;
      if (auto && !popupLike(el)) return;
      const text = (el.innerText || el.textContent || '').trim();
      if (text.length < 5 || text.length > 3200) return;
      if (!likelyError(text)) return;
      if (!list.some(x => x.text === text)) list.push({ text, rect: plainRect(el.getBoundingClientRect()) });
    });
    return list.slice(-8).reverse();
  }

  function renderErrors(items, auto = false) {
    const list = errPanel.querySelector('.list');
    errPanel.querySelector('.head b').textContent = auto ? '🩺 已自动翻译报错' : '🩺 错误码翻译';
    list.innerHTML = '';
    if (!items.length) {
      list.innerHTML = `<div class="item"><div class="t">没有找到明显错误码</div><div class="d">页面当前没有检测到 HTTP 错误、接口错误或网络错误。</div></div>`;
    } else {
      items.forEach((it) => {
        const hit = matchError(it.text);
        const div = document.createElement('div');
        div.className = 'item';
        div.innerHTML = `<div class="t"></div><div class="d"></div><div class="raw"></div>`;
        div.querySelector('.t').textContent = hit ? hit.t : '未知错误，正在自动翻译原文';
        div.querySelector('.d').textContent = hit ? hit.d : '正在调用 Edge 翻译引擎，请稍等…';
        div.querySelector('.raw').textContent = it.text;
        list.appendChild(div);
        if (!hit) {
          withNative('translate', { text: it.text }, (res) => {
            const d = div.querySelector('.d');
            const t = div.querySelector('.t');
            if (res && res.ok && res.text) {
              t.textContent = '未知错误 · 自动机翻';
              d.textContent = res.text;
            } else {
              t.textContent = '未知错误';
              d.textContent = '内置字典没有命中，机翻也失败。请复制原文检查接口或反代日志。';
            }
          });
        }
      });
    ...
    if (auto) showToast('已自动翻译红色报错');
  }

  let lastAutoErrorKey = '';
  let lastAutoErrorAt = 0;
  let autoScanTimer = null;
  const bootIgnoreUntil = Date.now() + 1600;

  function errorKey(items) {
  ...
  function autoErrorScan() {
    if (Date.now() < bootIgnoreUntil) return;
    const items = collectErrors(true);
    if (!items.length) return;
    const key = errorKey(items);
    if (key === lastAutoErrorKey && Date.now() - lastAutoErrorAt < 12000) return;
    lastAutoErrorKey = key;
    lastAutoErrorAt = Date.now();
    renderErrors(items.slice(0, 3), true);
  }

  function scheduleAutoErrorScan(delay = 220) {
    clearTimeout(autoScanTimer);
    autoScanTimer = setTimeout(autoErrorScan, delay);
  }

  const observer = new MutationObserver((mutations) => {
    for (const m of mutations) {
      if (m.target && m.target.closest?.('#dog-error-panel,#dog-actions,#dog-translation,#dog-card-panel,#dog-card-toast')) continue;
      if (m.type === 'childList' || m.type === 'characterData' || m.type === 'attributes') {
        scheduleAutoErrorScan();
        break;
      }
    }
  });
  observer.observe(document.documentElement, { childList:true, subtree:true, characterData:true, attributes:true, attributeFilter:['class','style','aria-hidden'] });
  setInterval(autoErrorScan, 2500);
  ...
  window.__tavernLiteTools = {
    ...
    scanErrors() { renderErrors(collectErrors(false), false); },
    hideAll() {
      actions.style.display = 'none';
