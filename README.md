...
- 为 `aaa.xixisillytavern.top` 增加明确的 ATS 原生 HTTP 例外。
- 修复原生 SSE 长连接报错 `NSURLErrorDomain -1022`。

## v2.0 单次声音与震动修复

- 回复结束后由 App 直接播放对应提示音，不再依赖画中画状态下不稳定的通知声音。
- 本地通知改为静音，只负责显示横幅。
- 每次生成结束只播放一次声音并震动一次，不再延迟重复提醒。

## v2.1 极速结束检测

- 恢复 v1.6 的 SillyTavern 原生生成结束事件，前台检测到后立即提醒。
- 服务端实时桥接继续作为网页被冻结后的后台兜底。
- 原生事件和服务端事件共用生成 ID，先到先提醒，后到自动去重。

## v2.2 严格单次提醒

- 没有真正收到生成开始时，忽略页面初始化产生的空回结束事件。
- 每轮生成只创建一个本地编号，原生事件、服务端桥接和轮询全部共用。
- 第一条结束事件提醒后立即关闭本轮，其余结束事件全部忽略。

## v2.3 微信式顶部横幅

- 完整回复：顶部显示“已完成回复”。
- 截断回复：顶部显示“回复已截断”，并提示建议重 Roll。
- 空回：顶部显示“本次已空回”，并提示建议重 Roll。
- 通知使用时间敏感级别，并提供“打开酒馆”操作。

## v2.4 横幅与重复提醒修复

- 通知延迟 0.2 秒投递，避开回复结束时的前后台/PiP 状态切换。
- 前台通知同时请求顶部横幅和通知中心展示。
- 提醒后增加 6 秒全局冷却锁，拦截结束后补发的伪开始和重复结束事件。


## v2.5 说明

本版关闭系统横幅、声音和震动提醒，避免多路检测造成重复弹出。回复完成、截断、空回会直接在画中画窗口内显示醒目的状态条。
