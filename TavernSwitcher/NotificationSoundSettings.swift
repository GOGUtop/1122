...
final class NotificationSoundSettings: ObservableObject {
    ...
    func soundURL(for outcome: ReplyOutcome) -> URL? {
        let name = defaults.string(forKey: key(for: outcome)) ?? builtInName(for: outcome)
        return try? soundsDirectory().appendingPathComponent(name)
    }

    func preview(outcome: ReplyOutcome) {
        guard let url = soundURL(for: outcome),
